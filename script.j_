...
if (taxiForm) {
    taxiForm.addEventListener('submit', function(e) {
        ...
        // Отправляем данные заказа на сервер для уведомления (в реальной реализации)
        // Здесь можно было бы добавить отправку POST-запроса на сервер
        console.log('Заказ такси получен. Необходимо уведомить администратора на номер 8 (918) 986-49-85');
        
        // Показываем информацию о необходимости ручного уведомления
        setTimeout(() => {
            alert('Важно: Пожалуйста, уведомите администратора на номер 8 (918) 986-49-85 о новом заказе.');
        }, 3000);
